...
# Convert text to TF-IDF features (Assuming you already trained a vectorizer)
vectorizer = TfidfVectorizer()
...
y_pred = ["happy", "sad", "happy", "sad"]  # Mock predictions (replace with `model.predict(X_test_tfidf)`)
...
def predict_emotion(change):
    ...
    if text.strip():  # Ensure input is not empty
        sample_tfidf = vectorizer.transform([text])  # Convert text to TF-IDF
        predicted_emotion = model.predict(sample_tfidf)[0]  # Predict emotion
